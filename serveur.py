...
async def bwira_abandi(socket, data):
    for umukinyi in sockets.values():
        print(umukinyi.id, socket.id)
        if umukinyi.id != socket.id:
            await umukinyi.send(data)
...
async def handler(websocket):
    global sockets
    if websocket.id not in sockets:
        sockets[websocket.id] = websocket
    try:
        async for message in websocket:
            await bwira_abandi(websocket, message)
    except websockets.exceptions.ConnectionClosedOK:
        del sockets[websocket.id]
        print("Connexion fermée proprement", locals())
    except Exception as e:
        print(f"Erreur : {e}")

async def main():
    async with websockets.serve(handler, "10.42.0.253", 8765):
